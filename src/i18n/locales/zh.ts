...
const zh: TranslationSchema = {
	...
	nav: {
		...
		codeQuality: '代码质量',
		codeQualityDesc: '9,600+测试，零失败，>90%覆盖率',
		aiReady: 'AI就绪',
		aiReadyDesc: '为编码代理打造',
	...
	home: {
		...
		heroHighlight: 'AI编排器',
		heroTagline:
			`用于部署受治理的多代理AI系统的开源生产就绪平台。配置驱动、自托管、合规就绪 — 内置可视化管理UI、65个MCP工具和企业安全。现已推出<strong>${EDDI_VERSION}</strong> (latest: ${EDDI_LATEST_VERSION})。`,
		problemTitle: '问题',
	...
	featuresOverview: {
		...
		cqCardTitle: '🧪 代码质量',
		cqCardDesc: '<strong>9,600+测试，零失败。</strong>零Checkstyle警告。>90%代码覆盖率。CI/CD在每次合并时强制执行。',
		aiCardTitle: '🧩 AI就绪',
	...
	pages: {
		...
		security: {
			...
			crisisPara: 'AI代理生态系统正面临前所未有的安全压力。2026年初，研究人员记录了主要开源代理框架中数百个严重漏洞。',
		...
		performance: {
			...
			benchTitle: '上下文中的性能',
			benchPara: '没有任何运行时能在所有场景中胜出。Java在涉及CPU密集型任务的场景中持续超越Node.js 30-68% — 这正是AI代理所需要的。',
			benchPara2: 'EDDI的架构专为混合I/O + CPU工作负载而优化：带有虚拟线程的Quarkus，结合Loom友好的连接池（Agroal）。',
		},
		multiAgent: {
			title: '多代理编排',
		...
		codeQuality: {
			title: '代码质量',
			description: '9,600+测试，零失败。零Checkstyle警告。>90%代码覆盖率。CI/CD质量门禁在每次合并时强制执行。',
			heading1: '工程严谨',
			para1: 'EDDI通过全面测试、静态分析和CI/CD质量门禁维持<strong>卓越的代码质量</strong>。每个拉取请求必须通过完整的测试套件才能合并。',
			heading2: '质量指标',
			items: [
				'<strong>9,600+测试</strong> — 单元、集成和端到端覆盖',
...
// ─── New Feature Pages ────────────────────────────────────
		...
		whyEddi: {
			...
			tcoDesc: '使用AI库的隐藏成本并非在于库本身 \u2014 it\'s the <strong>invisible infrastructure</strong> teams must build around it:',
			tcoBuildTitle: '使用库自建',
			tcoBuildItems: [
			...
			roiTitle: '商业案例',
			roiPara: 'EDDI的价值在于<strong>团队无需自行构建的部分</strong>.',
		...
		trust: {
			...
			timelineDesc: '在 LLM 时代之前，近二十年的持续工程。',
			timeline: [
