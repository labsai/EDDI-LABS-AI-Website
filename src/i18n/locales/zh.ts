...
const zh: TranslationSchema = {
	...
	gettingStarted: {
		...
		step3VaultTip: '<strong>💡 提示：</strong><code>${eddivault:my-anthropic-key}</code>语法引用EDDI内置的<a href="https://docs.labs.ai/security-and-compliance/secrets-vault" target="_blank" rel="noopener">Secrets Vault</a>。保险库主密钥由安装程序自动生成。如需快速测试，您也可以直接传递API密钥：<code>apiKey: "sk-ant-..."</code>。',
	...
	featuresOverview: {
		...
		cqCardTitle: '🧪 代码质量',
		cqCardDesc: '<strong>5,100+测试，零失败。</strong>零Checkstyle警告。CI/CD在每次合并时强制执行。',
		aiCardTitle: '🧩 AI就绪',
	...
	pages: {
		...
		codeQuality: {
			title: '代码质量',
			description: '5,100+测试，零失败。零Checkstyle警告。CI/CD质量门禁在每次合并时强制执行。',
			heading1: '工程严谨',
			para1: 'EDDI通过全面测试、静态分析和CI/CD质量门禁维持<strong>卓越的代码质量</strong>。每个拉取请求必须通过完整的测试套件才能合并。',
			heading2: '质量指标',
			items: [
				'<strong>5,100+测试</strong> — 单元、集成和端到端覆盖',
