...
const zh: TranslationSchema = {
	...
	nav: {
		...
		codeQuality: '代码质量',
		codeQualityDesc: '9,000+测试，零失败，>90%覆盖率',
		aiReady: 'AI就绪',
		aiReadyDesc: '为编码代理打造',
	...
	home: {
		...
		faq1Q: '什么是EDDI？',
		faq1A:
			'EDDI是一个开源的企业级AI编排平台。它使团队能够使用JSON配置而非编译代码来构建、配置和部署AI代理。EDDI提供了一个完整的平台，包括<strong>生产就绪的React管理界面</strong>（EDDI管理器）、内置REST API、对话状态管理、安全性（OIDC/Keycloak）、不可变审计跟踪和42个MCP工具——所有这些都可通过Docker或Kubernetes部署。',
		...
		faq4A: '模型上下文协议（MCP）是一个开放标准，允许Claude Desktop、IDE插件和自定义客户端等AI助手以编程方式与外部工具交互。EDDI公开了42个MCP工具，涵盖对话管理、代理管理、设置自动化、调度管理和诊断。',
	...
	gettingStarted: {
		...
		step2After: '现在您可以直接在AI助手中使用EDDI的42个MCP工具。',
		...
		nextMcpServerDesc: '浏览全部42个MCP工具',
		nextDocs: '文档 ↗',
	...
	featuresOverview: {
		title: '功能一览',
		description: '使EDDI成为企业AI编排器的所有功能——EDDI管理器UI、配置即代码、42个MCP工具、安全优先架构等。',
		heroTitle: '编排AI所需的',
		heroHighlight: '一切',
		...
		cqCardTitle: '🧪 代码质量',
		cqCardDesc: '<strong>9,000+测试，零失败。</strong>零Checkstyle警告。>90%代码覆盖率。CI/CD在每次合并时强制执行。',
		aiCardTitle: '🧩 AI就绪',
		aiCardDesc: '42个MCP工具，可预测的CRUD模式，自文档API。<strong>为编码代理</strong>而构建。',
	},
	pages: {
		manager: {
			title: 'EDDI管理器',
		...
		mcpServer: {
			...
			heading1: 'AI原生控制平面',
			para1: '模型上下文协议（MCP）是一个开放标准，允许AI助手与外部工具交互。EDDI实现了一个全面的MCP服务器，包含<strong>42个工具</strong>，覆盖平台的各个方面。',
			heading2: '工具类别',
			items: [
		...
		codeQuality: {
			title: '代码质量',
			description: '9,000+测试，零失败。零Checkstyle警告。>90%代码覆盖率。CI/CD质量门禁在每次合并时强制执行。',
			heading1: '工程严谨',
			para1: 'EDDI通过全面测试、静态分析和CI/CD质量门禁维持<strong>卓越的代码质量</strong>。每个拉取请求必须通过完整的测试套件才能合并。',
			heading2: '质量指标',
			items: [
				'<strong>9,000+测试</strong> — 单元、集成和端到端覆盖',
				'<strong>零失败</strong> — 每次构建CI清洁',
				'<strong>Checkstyle强制执行</strong> — 零警告，每次构建严格执行规则',
				'<strong>MCP工具测试</strong> — 所有42个MCP工具的全面覆盖',
				'<strong>CI/CD质量门禁</strong> — 自动化检查防止回归',
			],
		},
		aiReady: {
			title: 'AI就绪架构',
			description: '42个MCP工具，可预测的CRUD模式，自文档API。从头为编码代理而构建。',
			heading1: '为AI协作而设计',
			...
			items: [
				...
				'<strong>MCP优先</strong> — 42个为AI助手集成设计的工具',
...
// ─── New Feature Pages ────────────────────────────────────
		...
		whyEddi: {
			...
			items: [
				...
				'<strong>42个MCP工具</strong> — 通过模型上下文协议实现完整的AI原生控制',
		...
		unido: {
			...
			solutionDesc: 'EDDI 提供自托管、配置驱动的平台，用于部署治理良好的多代理 AI 系统 — 专为工业数字化设计。内置 42 个 MCP 工具、可视化管理和企业级安全，使全球南方组织能够在没有供应商锁定的情况下采用生产就绪的 AI 编排。',
