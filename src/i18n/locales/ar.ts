...
const ar: TranslationSchema = {
	...
	home: {
		...
		sol6Desc:
			'اكتشاف الوكلاء القائم على النية، والمحادثات المُدارة، و<strong>التعاون الجماعي</strong>: يتداول الوكلاء بسبعة أنماط، ويصوّتون، ويحررون نواتج العمل المشتركة معاً، ويمنحون البشر مقعداً على الطاولة.',
	...
	gettingStarted: {
		...
		step1ComposeOllama: 'تريد نموذجاً محلياً؟ أضف تراكب Ollama، وستبدأ الوكلاء الجديدة بعنوان URL الأساسي لـ Ollama مُعبّأً مسبقاً:',
		...
		step4Operator: 'تفضّل محادثة أو استمارة؟ يبدأ التثبيت الجديد دون أي وكلاء منشورين. افتح <strong>Platform Operator</strong> على <code>http://localhost:7070/manage/operator</code> وصِف الوكيل الذي تريده، مع انتظار كل تغيير لموافقتك، أو املأ استمارة معالج الوكلاء على <code>http://localhost:7070/manage/agents/wizard</code>.',
	...
	featuresOverview: {
		...
		multiCardDesc: 'اكتشاف قائم على النية، محادثات مُدارة، و<strong>تعاون جماعي</strong> مع التصويت ونواتج العمل المشتركة والبشر كأعضاء.',
	...
	pages: {
		...
		mcpServer: {
			...
			items: [
				...
				'<strong>أدوات المشاركة البشرية في الحلقة (10)</strong>: عرض الموافقات المعلقة وفحصها، استئناف المحادثات الموقوفة أو إلغاؤها، اعتماد مراحل المجموعة، وتقديم مداخلة العضو البشري في دوره',
		...
		security: {
			...
			items: [
				...
				'<strong>حواجز نتائج الأدوات</strong>: تحمل كل نتيجة أداة مصدرها، ويُوسَم النص ذو الطابع التوجيهي داخلها أو يُنقَّح أو يُطلَق تحذير بشأنه أو يُحظر قبل أن يتصرف النموذج بناءً عليه',
				...
				'<strong>حماية SSRF</strong>: يمنع التحقق من URL وقوائم النطاقات المسموح بها تزوير الطلبات من جانب الخادم، وتُرفض نقاط نهاية البيانات الوصفية السحابية كأهداف لاستدعاءات HTTP وMCP وA2A، بما في ذلك عبر تحليل DNS وإعادة التوجيه',
		...
		performance: {
			...
			items: [
				...
				'<strong>بث SSE</strong>: استجابات محادثة فورية وتغذية مناقشات جماعية وبث سجلات مباشر عبر Server-Sent Events، مع بث المخرجات رمزاً تلو الآخر حتى أثناء تشغيل الأدوات',
		...
		humanInTheLoop: {
			...
			opsItems: [
				...
				`<strong>اعتمد ما سيُنفَّذ فعلاً</strong>: ترتبط الموافقة بالطلب في صورته النهائية بعد تحليله، ويرى معتمِدو استدعاءات MCP وA2A الجهة المستهدفة وبصمة الطلب، لا اسم الأداة فقط.`,
				`<strong>Platform Operator خاضع لبوابة الموافقة</strong>: يستطيع Platform Operator إنشاء الوكلاء وتغييرها عبر API الخاص بـ EDDI، وتنتظر كل عملية كتابة يحاولها خلف بوابة الموافقة نفسها.`,
			],
		...
		whyEddi: {
			...
			items: [
				...
				'<strong>متعدد المستخدمين بالتصميم</strong>: مساحات عمل لكل مستخدم مع المشاركة، وConnections تتيح للوكلاء العمل ببيانات الاعتماد الخاصة بكل مستخدم',
