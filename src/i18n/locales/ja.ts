...
const ja: TranslationSchema = {
	...
	home: {
		...
		faq1Q: 'EDDIとは？',
		faq1A:
			'EDDIはオープンソースのエンタープライズAIオーケストレーションプラットフォームです。チームがコンパイルされたコードではなくJSON設定を使用してAIエージェントを構築、設定、デプロイすることを可能にします。EDDIは<strong>本番環境対応のReact管理UI</strong>（EDDIマネージャー）、組み込みREST API、会話状態管理、セキュリティ（OIDC/Keycloak）、不変の監査証跡、48のMCPツールを備えた完全なプラットフォームを提供します — すべてDockerまたはKubernetesでデプロイ可能。',
		...
		faq4A:
			'Model Context Protocol（MCP）は、Claude Desktop、IDEプラグイン、カスタムクライアントなどのAIアシスタントが外部ツールとプログラム的に対話できるオープンスタンダードです。EDDIは会話管理、エージェント管理、セットアップ自動化、スケジュール管理、診断にわたる48のMCPツールを公開しています。',
	...
	pages: {
		...
		mcpServer: {
			...
			para1:
				'Model Context Protocol（MCP）は、AIアシスタントが外部ツールと対話できるオープンスタンダード。EDDIはプラットフォームのあらゆる面をカバーする<strong>48のツール</strong>を備えた包括的MCPサーバーを実装しています。',
			heading2: 'ツールカテゴリ',
			items: [
