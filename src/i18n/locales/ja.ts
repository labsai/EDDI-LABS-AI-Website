...
const ja: TranslationSchema = {
	...
	nav: {
		...
		codeQuality: 'コード品質',
		codeQualityDesc: '5,100以上のテスト、失敗ゼロ',
		aiReady: 'AI対応',
	...
	gettingStarted: {
		...
		step3VaultTip: '<strong>💡 ヒント：</strong><code>${eddivault:my-anthropic-key}</code>構文はEDDI内蔵の<a href="https://docs.labs.ai/security-and-compliance/secrets-vault" target="_blank" rel="noopener">Secrets Vault</a>を参照します。Vaultマスターキーはインストーラーによって自動生成されます。簡単なテストの場合、APIキーを直接渡すこともできます：<code>apiKey: "sk-ant-..."</code>。',
	...
	featuresOverview: {
		...
		cqCardTitle: '🧪 コード品質',
		cqCardDesc: '<strong>5,100以上のテストで失敗ゼロ。</strong>Checkstyle警告ゼロ。CI/CDをすべてのマージで強制。',
		aiCardTitle: '🧩 AI対応',
	...
	pages: {
		...
		codeQuality: {
			title: 'コード品質',
			description: '5,100以上のテストで失敗ゼロ。Checkstyle警告ゼロ。CI/CD品質ゲートをすべてのマージで強制。',
			...
			heading2: '品質メトリクス',
			items: [
				'<strong>5,100以上のテスト</strong> — ユニット、統合、エンドツーエンドのカバレッジ',
